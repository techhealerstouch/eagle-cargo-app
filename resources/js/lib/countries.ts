...
/**
 * Validates a phone number based on the country dial code prefix.
 * Returns an error message string if invalid, or null if valid.
 */
export function validatePhone(phone: string, label: string = 'Phone number', defaultCountryCode: string = 'AU'): string | null {
    if (!phone) {
    ...
    // Check if it starts with any known country dial code from COUNTRIES
    // Sort by dial code length descending so longer codes (e.g. +1242) match before shorter ones (e.g. +1)
    const sorted = [...COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length);
    
    let matchedCountry: typeof COUNTRIES[number] | null = null;
    ...
    // If no dial code matched but it starts with + (e.g. invalid dial code)
    if (!matchedCountry && cleaned.startsWith('+')) {
        return `${label} must start with a valid country code`;
    }

    // Determine the country code (default to defaultCountryCode if no country matched and it doesn't start with +)
    const countryCode = matchedCountry ? matchedCountry.code : defaultCountryCode;
    const dialCode = matchedCountry ? matchedCountry.dialCode : COUNTRIES.find(c => c.code === defaultCountryCode)?.dialCode || '+61';

    if (countryCode === 'AU') {
        // Must match either +614XXXXXXXX or 04XXXXXXXX
        if (cleaned.startsWith('+61')) {
            if (!/^\+614\d{8}$/.test(cleaned)) {
                return `${label} must be a valid Australian mobile number (e.g. 04XX XXX XXX)`;
            }
        ...
        // Must match either +639XXXXXXXXX or 09XXXXXXXXX or 9XXXXXXXXX
        if (cleaned.startsWith('+63')) {
            if (!/^\+639\d{9}$/.test(cleaned)) {
                return `${label} must be a valid Philippine mobile number (e.g. 09XX XXX XXXX)`;
            }
        ...
        const national = cleaned.startsWith('+64') ? cleaned.slice(3) : cleaned.startsWith('0') ? cleaned.slice(1) : cleaned;
        if (!/^2\d{7,9}$/.test(national)) {
            return `${label} must be a valid New Zealand mobile number (e.g. 02X XXX XXXX)`;
        }
    } else if (countryCode === 'US' || countryCode === 'CA') {
        const national = cleaned.startsWith('+1') ? cleaned.slice(2) : cleaned;
        if (!/^[2-9]\d{9}$/.test(national)) {
            return `${label} must be a valid US/Canada phone number (10 digits)`;
        }
    } else if (countryCode === 'GB') {
        const national = cleaned.startsWith('+44') ? cleaned.slice(3) : cleaned.startsWith('0') ? cleaned.slice(1) : cleaned;
        if (!/^7\d{9}$/.test(national)) {
            return `${label} must be a valid UK mobile number (e.g. 07XXX XXXXXX)`;
        }
    } else if (countryCode === 'SG') {
        const national = cleaned.startsWith('+65') ? cleaned.slice(3) : cleaned;
        if (!/^[89]\d{7}$/.test(national)) {
            return `${label} must be a valid Singapore mobile number`;
        }
    } else {
        if (!/^\+\d{5,16}$/.test(cleaned)) {
            return `${label} must be a valid international phone number (e.g. ${dialCode}XXXXXXX)`;
        }
    }
