...
export default function DeliveriesIndex({
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Delivery Runsheets | Admin" />
                    ...
                    {runsheets.data.length > 0 ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center p-12">
                            ...
                            <h3 className="mb-2 font-serif text-xl font-medium text-brand-text">
                                No delivery runsheets yet
                            </h3>
                            <p className="mb-6 max-w-sm text-center text-brand-text-mid">
                                Create a runsheet to assign a courier to a
                                delivery route.
                            </p>
                            <Link
                                href="/admin/runsheets/create?type=delivery"
                        ...
                        <DialogHeader>
                            ...
                            {createdRunsheet && (
                                <DialogDescription className="text-brand-text-mid font-medium text-center text-sm">
                                    Assigned and ready for delivery on {new Date(createdRunsheet.scheduled_date).toLocaleDateString()}
                                </DialogDescription>
                            )}
                        </DialogHeader>
                            ...
                            <div className="w-full bg-brand-warm/10 rounded-xl p-4 grid grid-cols-2 gap-y-4 gap-x-2 text-left mb-2">
                                <div>
                                    ...
                                    <p className="font-bold text-brand-text text-sm">
                                        {createdRunsheet.boxes?.length > 0
                                            ? createdRunsheet.boxes[0].tracking_number || `BOX-${createdRunsheet.boxes[0].id}`
                                            : 'No boxes'}
                                        {createdRunsheet.boxes?.length > 1 && ` (+${createdRunsheet.boxes.length - 1})`}
                                    </p>
                                </div>
                                <div>
