import { Head, Link } from '@inertiajs/react';
import { ArrowLeft, Pencil, User, Mail, Phone, MapPin, Calendar, Package, Navigation, ExternalLink, ShieldCheck } from 'lucide-react';
import AppLayout from '@/layouts/app-layout';
import Pagination, { type PaginationData } from '@/components/common/pagination';
import { humanize } from '@/lib/utils';
import type { BreadcrumbItem } from '@/types';

interface RecipientDetail {
    id: number;
    name?: string | null;
    first_name?: string | null;
    last_name?: string | null;
    email?: string | null;
    phone_number?: string | null;
    secondary_phone_number?: string | null;
    address?: string | null;
    city?: string | null;
    province?: string | null;
    zip_code?: string | null;
    landmarks?: string | null;
    created_at?: string;
    sender?: {
        id: number;
        first_name: string;
        last_name: string;
        email?: string | null;
        mobile?: string | null;
        secondary_mobile?: string | null;
    } | null;
    area?: {
        id: number;
        name: string;
    } | null;
}

interface BoxItem {
    id: number;
    tracking_number?: string | null;
    serial_number?: string | null;
    status?: string | { value: string; label?: string };
    created_at?: string;
    booking?: { id: number; reference_number?: string } | null;
    box_type?: { name?: string } | null;
}

interface RecipientShowProps {
    recipient: RecipientDetail;
    boxes: PaginationData & { data: BoxItem[] };
    stats?: {
        totalBoxesCount?: number;
    };
}

const BOX_STATUS_COLORS: Record<string, string> = {
    pending: 'bg-amber-500/10 text-amber-600 border border-amber-500/20',
    assigned_picker: 'bg-indigo-500/10 text-indigo-600 border border-indigo-500/20',
    picked_up: 'bg-purple-500/10 text-purple-600 border border-purple-500/20',
    in_warehouse: 'bg-cyan-500/10 text-cyan-600 border border-cyan-500/20',
    shipped: 'bg-teal-500/10 text-teal-600 border border-teal-500/20',
    in_transit: 'bg-blue-500/10 text-blue-600 border border-blue-500/20',
    out_for_delivery: 'bg-blue-600/10 text-blue-700 border border-blue-600/20',
    delivered: 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/20',
    cancelled: 'bg-rose-500/10 text-rose-600 border border-rose-500/20',
};

export default function RecipientShow({ recipient, boxes, stats = {} }: RecipientShowProps) {
    const fullName = recipient.name || `${recipient.first_name || ''} ${recipient.last_name || ''}`.trim() || `Recipient #${recipient.id}`;
    const fullAddress = [recipient.address, recipient.city, recipient.province, recipient.zip_code]
        .filter(Boolean)
        .join(', ');
    const senderName = recipient.sender ? `${recipient.sender.first_name} ${recipient.sender.last_name}`.trim() : 'N/A';

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Recipients Directory', href: '/admin/recipients' },
        { title: fullName, href: `/admin/recipients/${recipient.id}` },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${fullName} | Recipient Details`} />
            <div className="flex h-full flex-1 flex-col gap-6 p-4 sm:p-6 md:p-8 min-w-0 w-full">
                {/* Header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brand-warm/20 pb-6">
                    <div className="flex items-center gap-4">
                        <Link
                            href="/admin/recipients"
                            className="h-10 w-10 rounded-xl border border-zinc-200 bg-white text-zinc-600 hover:text-zinc-900 hover:bg-zinc-50 flex items-center justify-center transition-all shadow-2xs"
                        >
                            <ArrowLeft className="size-5" />
                        </Link>
                        <div>
                            <div className="flex items-center gap-3">
                                <h1 className="font-sans text-2xl font-bold tracking-tight text-brand-text">{fullName}</h1>
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-700 border border-cyan-500/20">
                                    Recipient Profile
                                </span>
                            </div>
                            <p className="text-xs font-mono text-zinc-500 mt-1">
                                RECIPIENT #{recipient.id} {recipient.created_at && `• Created on ${new Date(recipient.created_at).toLocaleDateString()}`}
                            </p>
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        <Link
                            href={`/admin/recipients/${recipient.id}/edit`}
                            className="bg-brand-rust text-white hover:bg-brand-rust/90 flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest shadow-md transition-all font-sans"
                        >
                            <Pencil className="size-4" />
                            Edit Recipient Profile
                        </Link>
                    </div>
                </div>

                {/* Main Content Grid */}
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Primary Recipient Details Card */}
                    <div className="lg:col-span-1 flex flex-col gap-6">
                        <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xs">
                            <div className="flex items-center gap-4 border-b border-zinc-100 pb-5 mb-5">
                                <div className="h-14 w-14 rounded-2xl bg-cyan-50 text-cyan-700 border border-cyan-100 flex items-center justify-center font-bold text-xl">
                                    {fullName.charAt(0).toUpperCase()}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h2 className="font-bold text-brand-text truncate text-base">{fullName}</h2>
                                    <p className="text-xs text-zinc-500 truncate flex items-center gap-1.5 mt-0.5">
                                        <Navigation className="size-3.5 text-cyan-600" />
                                        {recipient.area?.name || <span className="italic text-zinc-400">Unassigned Area</span>}
                                    </p>
                                </div>
                            </div>

                            <div className="space-y-4 text-xs font-normal">
                                <div>
                                    <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Recipient Reference ID</span>
                                    <span className="font-mono font-semibold text-zinc-900 bg-zinc-50 px-2.5 py-1 rounded-md border border-zinc-200/60 inline-block">
                                        REC-{recipient.id}
                                    </span>
                                </div>

                                <div>
                                    <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Primary Phone</span>
                                    <span className="font-medium text-zinc-800 flex items-center gap-1.5 font-mono text-xs">
                                        <Phone className="size-3.5 text-zinc-400" />
                                        {recipient.phone_number || <span className="text-zinc-400 italic font-sans">Not provided</span>}
                                    </span>
                                </div>

                                {recipient.secondary_phone_number && (
                                    <div>
                                        <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Secondary Phone</span>
                                        <span className="font-medium text-zinc-800 flex items-center gap-1.5 font-mono text-xs">
                                            <Phone className="size-3.5 text-zinc-400" />
                                            {recipient.secondary_phone_number}
                                        </span>
                                    </div>
                                )}

                                <div>
                                    <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Delivery Address</span>
                                    <span className="font-medium text-zinc-800 flex items-start gap-1.5">
                                        <MapPin className="size-3.5 text-zinc-400 shrink-0 mt-0.5" />
                                        <span>{fullAddress || <span className="text-zinc-400 italic">Not provided</span>}</span>
                                    </span>
                                </div>

                                {recipient.landmarks && (
                                    <div>
                                        <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Landmarks & Notes</span>
                                        <div className="bg-zinc-50 border border-zinc-200/60 rounded-xl p-3 text-zinc-700 text-xs mt-1">
                                            {recipient.landmarks}
                                        </div>
                                    </div>
                                )}

                                {recipient.created_at && (
                                    <div>
                                        <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 block mb-1">Created Date</span>
                                        <span className="font-medium text-zinc-800 flex items-center gap-1.5">
                                            <Calendar className="size-3.5 text-zinc-400" />
                                            {new Date(recipient.created_at).toLocaleString()}
                                        </span>
                                    </div>
                                )}
                            </div>
                        </div>

                        {/* Associated Sender Card */}
                        {recipient.sender && (
                            <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xs">
                                <h3 className="font-bold text-brand-text text-sm uppercase tracking-wider mb-4 flex items-center gap-2">
                                    <User className="size-4 text-brand-rust" />
                                    Associated Sender Profile
                                </h3>
                                <div className="bg-brand-warm/10 border border-brand-warm/20 rounded-xl p-4 flex flex-col gap-3">
                                    <div>
                                        <p className="font-bold text-brand-text text-sm">{senderName}</p>
                                        {recipient.sender.email && (
                                            <p className="text-xs text-zinc-500 mt-0.5">{recipient.sender.email}</p>
                                        )}
                                        {recipient.sender.mobile && (
                                            <p className="text-xs font-mono text-zinc-600 mt-0.5">{recipient.sender.mobile}</p>
                                        )}
                                    </div>
                                    <Link
                                        href={`/admin/senders/${recipient.sender.id}`}
                                        className="text-xs font-bold text-brand-rust hover:underline inline-flex items-center gap-1 mt-1"
                                    >
                                        View Sender Details <ExternalLink className="size-3" />
                                    </Link>
                                </div>
                            </div>
                        )}
                    </div>

                    {/* Boxes History */}
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        {/* Overview Stats */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xs flex items-center justify-between">
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Total Boxes Shipped To Recipient</p>
                                    <p className="font-bold text-brand-text text-2xl mt-1">{stats.totalBoxesCount ?? 0}</p>
                                </div>
                                <div className="h-12 w-12 rounded-xl bg-cyan-50/80 text-cyan-600 border border-cyan-100 flex items-center justify-center">
                                    <Package className="size-6" />
                                </div>
                            </div>

                            <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xs flex items-center justify-between">
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-wider text-zinc-400">Delivery Region / Area</p>
                                    <p className="font-bold text-zinc-900 text-lg mt-1 truncate">{recipient.area?.name || 'Standard Area'}</p>
                                </div>
                                <div className="h-12 w-12 rounded-xl bg-purple-50/80 text-purple-600 border border-purple-100 flex items-center justify-center">
                                    <Navigation className="size-6" />
                                </div>
                            </div>
                        </div>

                        {/* Associated Boxes Table */}
                        <div className="rounded-2xl border border-zinc-200/80 bg-white overflow-hidden shadow-2xs">
                            <div className="p-6 border-b border-zinc-100 flex items-center justify-between">
                                <h3 className="font-bold text-brand-text text-sm uppercase tracking-wider flex items-center gap-2">
                                    <Package className="size-4 text-brand-rust" />
                                    Box Delivery Records
                                </h3>
                            </div>

                            {boxes.data.length > 0 ? (
                                <div className="overflow-x-auto w-full">
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="border-b border-zinc-200/80 bg-zinc-50/70 text-[11px] font-semibold text-zinc-600 uppercase tracking-wider">
                                                <th scope="col" className="px-6 py-3 font-semibold">Tracking #</th>
                                                <th scope="col" className="px-6 py-3 font-semibold">Box Type</th>
                                                <th scope="col" className="px-6 py-3 font-semibold">Status</th>
                                                <th scope="col" className="px-6 py-3 font-semibold">Booking Ref</th>
                                                <th scope="col" className="px-6 py-3 font-semibold text-right">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-zinc-100 text-xs font-normal">
                                            {boxes.data.map((box) => {
                                                const statusVal = typeof box.status === 'object' && box.status !== null ? box.status.value : String(box.status || 'pending');
                                                const statusColor = BOX_STATUS_COLORS[statusVal] || 'bg-zinc-100 text-zinc-700 border-zinc-200';
                                                return (
                                                    <tr key={box.id} className="hover:bg-zinc-50/60 transition-colors">
                                                        <td className="px-6 py-3.5 font-mono font-semibold text-zinc-900">
                                                            {box.tracking_number || box.serial_number || `#${box.id}`}
                                                        </td>
                                                        <td className="px-6 py-3.5 text-zinc-700">
                                                            {box.box_type?.name || 'Standard Balikbayan Box'}
                                                        </td>
                                                        <td className="px-6 py-3.5">
                                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold uppercase tracking-wider ${statusColor}`}>
                                                                {humanize(statusVal)}
                                                            </span>
                                                        </td>
                                                        <td className="px-6 py-3.5 font-mono text-zinc-600">
                                                            {box.booking ? (
                                                                <Link href={`/admin/bookings/${box.booking.id}`} className="hover:underline text-brand-rust font-semibold">
                                                                    {box.booking.reference_number || `#${box.booking.id}`}
                                                                </Link>
                                                            ) : (
                                                                '—'
                                                            )}
                                                        </td>
                                                        <td className="px-6 py-3.5 text-right whitespace-nowrap">
                                                            {box.booking && (
                                                                <Link
                                                                    href={`/admin/bookings/${box.booking.id}`}
                                                                    className="text-xs font-bold text-brand-rust hover:underline inline-flex items-center gap-1"
                                                                >
                                                                    View Booking <ExternalLink className="size-3" />
                                                                </Link>
                                                            )}
                                                        </td>
                                                    </tr>
                                                );
                                            })}
                                        </tbody>
                                    </table>
                                    <div className="border-t border-zinc-100">
                                        <Pagination data={boxes} />
                                    </div>
                                </div>
                            ) : (
                                <div className="p-8 text-center text-xs text-zinc-400 italic">
                                    No boxes found recorded under this recipient.
                                </div>
                            )}
                        </div>
                    </div>
                </div>
            </div>
        </AppLayout>
    );
}
